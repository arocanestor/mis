...
export default class DemoApp extends Component {
  ...
  componentDidMount() {
    setInterval( () =>  this.numeroMisasDis() ,
      10
    );
  }
            ...
            <tbody>
              {this.state.misas.map(misa => (
                <tr key={misa._id}>
                  ...
                  <td> {this.state.login?
                        <button className="btn btn-danger " onClick={() => this.eliminarMisa(misa._id)} >
                        Delete
                       </button>
                       :
                       ''}                    
    ...
    return (
      <div className="justify-content-center">
        <Modal isOpen={this.state.newMisa} >
          ...
          <ModalFooter>
            <Button type="submit" onClick={this.crearMisa} className="btn btn-primary">Crear</Button>
        ...
        return (
          <div >
            <div className="custom-control custom-radio custom-control-inline">
              <input id='1' type="radio" value="1" name="hora" className="custom-control-input" onChange={this.inputChange}></input>
              <label className="custom-control-label" htmlFor="1" >Mañana</label>
            </div>
            <div className="custom-control custom-radio custom-control-inline">
              <input id='2' type="radio" value="2" name="hora" className="custom-control-input" onChange={this.inputChange}></input>
              <label className="custom-control-label" htmlFor="2">Tarde</label>
            </div>
          </div>
        )
      } else {
  ...
  botonNuevaMisa = () => {
    if (this.state.diaFestivo) {
      if (this.state.dispon || this.state.dispon2) {
        return (
          <div>
            <button className="btn btn-danger" onClick={this.activarM}>Nueva misa</button>
            <br /><br />
          ...
          <div className="col-md-auto">
            <div>
              <div>
                ...
                <button className="btn btn-danger" onClick={this.botonLista}>Exporta lista</button>
              </div>
            </div>
          </div>
        </div>
