...
router.options("/", (req, res) => {
  ...
  res.setHeader(
    ...
    "access-control-allow-origin, access-control-allow-headers,access-control-allow-origin"
  );
  res.status(200);
  res.json("opciones:ok");
});
...
router.get("/:dia", async (req, res) => {
  ...
  for (let i = 0; lista.length > i; i++) {
    ws.cell(i + 2, 1)
      .string(lista[i].ofrece)
      .style(style);
    ws.cell(i + 2, 2)
      .string(lista[i].intencion)
      .style(style);
    ws.cell(i + 2, 3)
      .string(JSON.stringify(lista[i].dia))
      .style(style);
    ws.cell(i + 2, 4)
      .string(lista[i].tel)
      .style(style);
    ws.cell(i + 2, 5)
      .string(horaDeMisa(lista[i].hora))
      .style(style);
    ws.cell(i + 2, 6)
      .string(lista[i].para)
      .style(style);
